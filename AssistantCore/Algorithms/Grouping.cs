...
namespace AssistantCore.Algorithms
{
    public class Groupings
    {
        ...
        public /*IEnumerable<IGrouping<int, string>>*/int DivideOnPatches(List<string> source, int patchSize)
        {
            int key = 0;

            IEnumerable<KeyValuePair<int, string>> result = source.Select((value, index) =>
            {
                if (index % patchSize == 0)
                {
                    key++;
                }
            ...
            /*return */var packages = result.GroupBy(p => p.Key, t => t.Value);

            foreach (var pack in packages)
            {
                Console.WriteLine($"Package {pack.Key} in proccess");
                ProccessPack(pack);
            }
        ...
        private static void ProccessPack(IGrouping<int, string> pack)
        {
            IEnumerable<string> items = pack.Select(m => m);
